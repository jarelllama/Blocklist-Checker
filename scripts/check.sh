...
# Generates daily CSV and markdown reports for the given blocklist.
...
# Generate daily CSV report
generate_daily_csv_report() {
    local report_filename="$REPORTS_DIR/$(date +'%Y-%m-%d')_report.csv"
    echo "Title, Raw Count, Compressed Count, Compression Percentage, Dead Percentage, Invalid Entries Count, Invalid Entries Percentage, Usable Percentage, In Tranco Count" > "$report_filename"
    echo "$title, $raw_count, $compressed_count, $compression_percentage%, $dead_percentage%, $invalid_entries_count, $invalid_entries_percentage%, $usable_percentage%, $in_tranco_count" >> "$report_filename"
}
